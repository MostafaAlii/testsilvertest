    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{route('trunkModel.store')}}" method="POST">
                    @csrf
                    <!-- Start Name -->
                    ...
                    <div class="form-group p-1">
                        ...
                        <select name="trunk_make_id" class="form-control">
                            <option selected disabled>Select {{$data['title']}} Truck Make...</option>
                            @forelse ($data['trunk_makes'] as $trunk_make)
                            <option value="{{$trunk_make->id}}" {{ old('trunk_make')==$trunk_make->id ? 'selected' : '' }}>{{
                                $trunk_make->name }}
                            </option>
                            @empty
                            <option value="0">No Truck Make</option>
                            @endforelse
                        </select>
                        @error('trunk_make_id')
                        <div class="alert alert-danger">{{ $message }}</div>
