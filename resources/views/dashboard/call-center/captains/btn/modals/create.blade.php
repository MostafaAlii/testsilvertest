        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{route('CallCenterCaptains.store')}}" method="POST" enctype="multipart/form-data">
                    ...
                    <!-- Start status caption type Status -->
                    <div class="p-1 form-group">
                        <label for="status_caption_type">Captain Type</label>
                        <select name="status_caption_type" class="form-control">
                            <option selected disabled>Select {{$data['title']}} Type...</option>
                            <option value="car" {{ old('status_caption_type') == 'car' ? 'selected' : '' }}>Car</option>
                            <option value="scooter" {{ old('status_caption_type') == 'scooter' ?'selected' : '' }}>Scooter</option>
                            <option value="tricycle" {{ old('status_caption_type') == 'tricycle' ?'selected' : '' }}>Tricycle</option>
                            <option value="trunk" {{ old('status_caption_type') == 'trunk' ?'selected' : '' }}>Trunk</option>
                        </select>
                        @error('status_caption_type')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <!-- End Status Selected -->
